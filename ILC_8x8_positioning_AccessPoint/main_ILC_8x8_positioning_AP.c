...
int main(void)
{
 ...
 // __bis_SR_register(LPM3_bits + GIE);
  ...
  CCTL0 |= CCIE; 
  _EINT();
  //low_power_delay( 44000);
   
//  CCTL0 |= CCIE;
//  TACCTL0=0;
    //slow timeout, ~100msec
...
void MRFI_RxCompleteISR()
{
  ...
  if(packet_dn.frame[9] == 0xfb){
    packet_dn.frame[18] = timer&0xff;
    packet_dn.frame[19] = (timer >> 8) & 0xff;
    
    packet_dn.frame[20] = timer&0xff;
    packet_dn.frame[21] = (timer >> 8) & 0xff;
  }
  // packet_dn.frame[9] = 0xFB;
...
void send_sync_packet(){
  ...
  sync_packet.frame[0] = 8+6; 
  sync_packet.frame[9] =  SYNC_PACKET;
  sync_packet.frame[10] = timer&0xff;
  sync_packet.frame[11] = (timer >> 8) & 0xff;  
  sync_packet.frame[12] = timer&0xff;
  sync_packet.frame[13] = (timer >> 8) & 0xff;
  sync_packet.frame[14] = inc;  
